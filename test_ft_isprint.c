...
void test_ft_isprint(void)
{
    ...
    for (int c = 0; c <= 127; c++) {
        // Compare both as boolean values (zero or non-zero)
        if ((ft_isprint(c) != 0) != (isprint(c) != 0)) {
            printf("  FAIL: ft_isprint(%d/'%c') = %d, expected %s\n",
                   c, isprint(c) ? c : ' ', ft_isprint(c),
                   isprint(c) ? "non-zero" : "zero");
            failed++;
        }
    }
    ...
    for (int c = 128; c <= 255; c++) {
        if ((ft_isprint(c) != 0) != (isprint(c) != 0)) {
            printf("  FAIL: ft_isprint(%d) = %d, expected %s\n",
